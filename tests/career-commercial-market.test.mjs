...
test('development layout is one attribute card per row and two specialty cards per row on desktop',()=>{
  ...
  assert.match(developmentLayout,/\.attribute-grid\{[\s\S]*grid-template-columns:minmax\(0,1fr\)/);
  assert.match(developmentLayout,/\.profile-skill-list\{[\s\S]*grid-template-columns:repeat\(2,minmax\(0,1fr\)\)/);
  assert.match(developmentLayout,/@media\(max-width:820px\)\{[\s\S]*profile-skill-list\{grid-template-columns:1fr\}/);
});
