...
function enhanceDecision(engine,payload){
  const key=situationKey(engine),rows=CATALOG[key]||CATALOG.support_move,ctx=contextFor(engine);let regular=shuffle(rows,engine.rng).slice(0,3).map(row=>contextualize(engine,makeChoice(engine,row),ctx));
  if(!regular.some(choice=>choice.energyCost<=2)){const economical=rows.find(row=>Number(row[4])<=2);if(economical)regular[2]=contextualize(engine,makeChoice(engine,economical),ctx);}
  ...
  return{...payload,title,options:regular,energy:engine.user?.energy,gameplayContext:ctx,intensity:engine.matchIntensity||'moderate',rareAppearanceChance:Math.round(rareChance(engine,key)*100)};
}
