...
const INTENSITY={
  light:{label:'Leve',momentGate:.62,nextMoment:2,minuteRefund:.58,idleRecovery:.18,extraDrain:0},
  moderate:{label:'Moderada',momentGate:1,nextMoment:0,minuteRefund:.08,idleRecovery:0,extraDrain:0},
  intense:{label:'Intensa',momentGate:1.16,nextMoment:-1,minuteRefund:0,idleRecovery:0,extraDrain:.18}
};
...
function opponentBase(engine){return Number(engine.directOpponent?.ovr||active(teamPlayers(engine,opponentTeam(engine.user?.team||'home'))).reduce((s,p)=>s+Number(p.ovr||60),0)/Math.max(1,active(teamPlayers(engine,opponentTeam(engine.user?.team||'home'))).length)||60);}
...
function shuffle(list,rng){const out=[...list];for(let i=out.length-1;i>0;i--){const j=Math.floor(rng()*(i+1));[out[i],out[j]]=[out[j],out[i]];}return out;}
function contextFor(engine){const user=engine.user;if(!user)return{pressure:50,space:50,angle:50,markers:1,progress:50};const opponents=active(teamPlayers(engine,opponentTeam(user.team)));const distances=opponents.map(player=>Math.hypot(player.x-user.x,player.y-user.y)).sort((a,b)=>a-b);const nearest=distances[0]??18;const markers=distances.filter(distance=>distance<13).length;const pressure=clamp(78-nearest*4.2+markers*7+(engine.playState?.pressureBias||0),5,96);const progress=progressOf(engine,user.team,user.x);const angle=clamp(100-Math.abs(user.y-50)*1.45-(progress<70?(70-progress)*.35:0),8,100);return{pressure,space:clamp(100-pressure+(engine.playState?.advantage||0)*.7,4,98),angle,markers,progress};}
...
function contextualize(engine,choice,ctx){const c={...choice,tags:[...(choice.tags||[])]};let difficulty=Number(c.difficulty||opponentBase(engine));const advantage=Number(engine.playState?.advantage||0);if(c.tags.includes('shot')){difficulty+=ctx.pressure*.10+ctx.markers*2.2+(100-ctx.angle)*.045;if(ctx.progress>86)difficulty-=Math.min(6,(ctx.progress-86)*.45);}else if(c.tags.includes('dribble'))difficulty+=ctx.pressure*.055+ctx.markers*1.5;else if(c.tags.includes('pass')||c.tags.includes('chance'))difficulty+=ctx.pressure*.035+ctx.markers*.8;else if(c.tags.includes('cross'))difficulty+=ctx.pressure*.03;else if(c.tags.includes('run')||c.tags.includes('sprint'))difficulty+=ctx.pressure*.025;if(['shot','dribble','pass','chance','cross','run','sprint'].some(tag=>c.tags.includes(tag)))difficulty-=advantage*.28;c.difficulty=difficulty;c.context=ctx;c.skillValue=Math.round(skillValue(engine,c.skill));const readiness=(engine.userPreparation||70)/100,energy=(engine.user?.energy||70)/100,mental=(engine.mentalStability||70)/100;const chemistry=c.tags.includes('pass')?Number(engine.user?.chemistry||0)*.04:0;const deterministic=c.skillValue*.58+(readiness*.22+energy*.18+mental*.12)*100-difficulty*.38-Number(c.cost||0)*.12+scoreContext(engine,c)+instructionContext(engine,c)+chemistry;const threshold=c.tags.includes('shot')?(c.tags.includes('penalty')?55:c.tags.includes('free_kick')?62:61):47;c.successChance=Math.round(clamp(((deterministic+6.5-threshold)/13)*100,1,99));c.chanceLabel=c.tags.includes('shot')?'Gol':'Sucesso';c.energyCost=Math.max(0,Number(c.cost||0));c.riskLabel=c.successChance>=72?'Favorável':c.successChance>=48?'Equilibrada':c.successChance>=28?'Difícil':'Muito difícil';return c;}
function situationKey(engine){const key=engine.pendingDecision?.situation?.key;if(key&&key!=='chain')return key;const last=engine.playState?.lastChoice;if(last?.tags?.includes('cross'))return'off_ball_attack';if(last?.tags?.includes('dribble')||last?.tags?.includes('shot')||last?.tags?.includes('run'))return'box_ball';return'central_ball';}
...
function enhanceDecision(engine,payload){const key=situationKey(engine),rows=CATALOG[key]||CATALOG.support_move;const ctx=contextFor(engine);let regular=shuffle(rows,engine.rng).slice(0,3).map(row=>contextualize(engine,makeChoice(engine,row),ctx));const safe=regular.some(choice=>choice.energyCost<=2);if(!safe){const economical=rows.find(row=>Number(row[4])<=2);if(economical)regular[2]=contextualize(engine,makeChoice(engine,economical),ctx);}const specialRows=SPECIALS[key]||[];if(specialRows.length&&engine.rng()<rareChance(engine,key)){const special=contextualize(engine,makeChoice(engine,pick(specialRows,engine.rng)),ctx);special.rare=true;regular.push(special);}const title=pick(TITLES[key]||[payload.title],engine.rng)||payload.title;engine.pendingDecision.options=regular;engine.pendingDecision.situation={...(engine.pendingDecision.situation||{}),key,title};engine.playState.currentContext=ctx;return{...payload,title,options:regular,energy:engine.user?.energy,gameplayContext:ctx,intensity:engine.matchIntensity||'moderate',rareAppearanceChance:Math.round(rareChance(engine,key)*100)};}
...
if(!CareerMatchEngine.prototype.__gameplayDepthInstalled){
 ...
 CareerMatchEngine.prototype.start=function(...args){this.matchIntensity='moderate';this.playState={advantage:0,pressureBias:0,sequence:0,lastChoice:null,currentContext:null};this.intensityMinutes={light:0,moderate:0,intense:0};this.decisionHistory=[];this.energySpentActions=0;const startEnergy=Number(this.user?.energy??this.context?.state?.energy??80);this.matchRecoveryCeiling=Math.min(92,startEnergy+6);const result=originalStart.apply(this,args);if(typeof window!=='undefined')window.dispatchEvent(new CustomEvent('career:match-engine-ready',{detail:{engine:this}}));return result;};
 CareerMatchEngine.prototype.shouldUserMoment=function(){const mode=this.matchIntensity||'moderate',cfg=INTENSITY[mode]||INTENSITY.moderate;let eligible=originalShouldMoment.call(this);if(mode==='intense'&&!eligible&&this.user?.onField&&!this.awaitingDecision&&this.userMoments<this.maxUserMoments&&this.minute>=this.nextUserMoment&&this.rng()<.16)eligible=true;if(!eligible)return false;return mode==='light'?this.rng()<cfg.momentGate:true;};
 CareerMatchEngine.prototype.openUserMoment=function(...args){const result=originalOpenMoment.apply(this,args),cfg=INTENSITY[this.matchIntensity||'moderate']||INTENSITY.moderate;if(this.nextUserMoment!=null)this.nextUserMoment=Math.max(this.minute+2,this.nextUserMoment+cfg.nextMoment);return result;};
 CareerMatchEngine.prototype.choose=function(key){const choice=this.pendingDecision?.options?.find(option=>option.key===key);if(!choice)return originalChoose.call(this,key);const before={goals:Number(this.playerStats?.goals||0),energy:Number(this.user?.energy||0)};const result=originalChoose.call(this,key);const afterEnergy=Number(this.user?.energy||0);this.energySpentActions+=Math.max(0,before.energy-afterEnergy);if(choice.energyCost<=2&&result?.success&&['pass','support','defend'].some(tag=>choice.tags?.includes(tag))&&this.user?.onField)this.user.energy=clamp(this.user.energy+.35,0,this.matchRecoveryCeiling);this.decisionHistory.push({minute:this.minute,key:choice.key,label:choice.label,success:Boolean(result?.success),chance:choice.successChance,cost:choice.energyCost,energy_before:Math.round(before.energy),energy_after:Math.round(this.user?.energy??afterEnergy),intensity:this.matchIntensity||'moderate',context:choice.context});if(this.decisionHistory.length>40)this.decisionHistory.shift();updateSequence(this,choice,result,before);maybeContinuation(this,choice,result,before);return result;};
 ...
 CareerMatchEngine.prototype.result=function(...args){const result=originalResult.apply(this,args);if(result?.playerStats){result.playerStats.intensity_minutes={...this.intensityMinutes};result.playerStats.energy_spent_actions=Math.round(this.energySpentActions||0);result.playerStats.decision_count=this.decisionHistory.length;result.playerStats.rare_actions=this.decisionHistory.filter(item=>String(item.key).startsWith('sp_')).length;result.playerStats.final_intensity=this.matchIntensity||'moderate';result.playerStats.decision_history=this.decisionHistory.slice(-30);}return result;};
 Object.defineProperty(CareerMatchEngine.prototype,'__gameplayDepthInstalled',{value:true,enumerable:false,configurable:false});
}
