import { signUpUser, signInUser, resetPassword, getCurrentSession } from '../../services/auth-service.js';
import { traduzirErro } from '../../utils/errors.js';
import { showToast } from '../../components/toast/toast.js';
  ...
  document.getElementById('loginForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      showToast('Login efetuado', 'Preparando ambiente do jogo...', 'sucesso');
      window.location.href = 'dashboard.html'; // Substitua pela próxima tela
    } catch (error) {
      showToast('Acesso Negado', traduzirErro(error.message), 'erro');
    }
  });
  ...
  document.getElementById('cadastroForm').addEventListener('submit', async (e) => {
    ...
    if (!aceite) {
      showToast('Atenção', 'Você deve aceitar os Termos e Políticas para jogar.', 'aviso');
      return;
    }

    if (senha !== confirmSenha) {
      showToast('Atenção', 'As senhas não conferem.', 'erro');
      return;
    }

    try {
      await signUpUser(email, senha, username);
      showToast('Contrato Assinado!', 'Cadastro realizado com sucesso. Verifique seu e-mail para validar sua entrada no mundo.', 'sucesso');
      // Pode redirecionar ou trocar a aba para login
    } catch (error) {
      showToast('Erro no Registro', traduzirErro(error.message), 'erro');
    }
  });
  ...
  if (recuperarForm) {
    recuperarForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        showToast('Recuperação Iniciada', 'As instruções foram enviadas para o seu e-mail.', 'sucesso');
      } catch (error) {
        showToast('Falha', traduzirErro(error.message), 'erro');
      }
    });
  }
