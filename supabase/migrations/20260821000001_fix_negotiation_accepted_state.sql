-- Correção segura: preservar a função viva e separar accepted/countered.
-- Gerada a partir da definição consultada no Supabase em 2026-08-21.
...
BEGIN
  ...
  BEGIN
    v_curr_wage:=(v_offer.current_terms->>'monthly_wage')::int;v_req_wage:=(p_requested_terms->>'monthly_wage')::int;
    v_curr_dur:=(v_offer.current_terms->>'duration_seasons')::int;v_req_dur:=(p_requested_terms->>'duration_seasons')::int;
    v_curr_release:=(v_offer.current_terms->>'release_clause')::int;v_req_release:=(p_requested_terms->>'release_clause')::int;
    v_curr_role:=v_offer.current_terms->>'squad_role';v_req_role:=p_requested_terms->>'squad_role';v_curr_bonus:=coalesce((v_offer.current_terms->>'signing_bonus')::int,0);
  ...
  IF v_role_diff>0 THEN v_cost:=v_cost+(v_role_diff*25);ELSIF v_role_diff<0 THEN v_cost:=v_cost+(v_role_diff*15);END IF;
  IF v_req_dur<v_curr_dur THEN v_cost:=v_cost+((v_curr_dur-v_req_dur)*5);ELSIF v_req_dur>v_curr_dur THEN v_cost:=v_cost-((v_req_dur-v_curr_dur)*5);END IF;
  ...
  INSERT INTO public.player_offer_history(offer_id,round,requested_terms,response_action,club_response_terms,previous_terms,negotiation_cost,remaining_flexibility,before_stance,after_stance)
  VALUES(p_offer_id,v_round,v_requested_terms,v_response_action,v_response_terms,v_offer.current_terms,v_cost,v_flex_rem,CASE WHEN v_offer.status='countered' THEN 'cauteloso' ELSE 'interessado' END,v_club_stance);
  IF v_response_action='accepted' THEN UPDATE public.player_offers SET status='accepted',current_terms=v_response_terms,round=v_round,internal_tolerance=v_flex_rem WHERE id=p_offer_id; ELSIF v_response_action='withdrawn' THEN UPDATE public.player_offers SET status='withdrawn',round=v_round,internal_tolerance=v_flex_rem WHERE id=p_offer_id; ELSE UPDATE public.player_offers SET status='countered',current_terms=v_response_terms,round=v_round,internal_tolerance=v_flex_rem WHERE id=p_offer_id; END IF;
