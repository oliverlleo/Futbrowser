...
DECLARE
  ...
  v_new text := 'IF coalesce(p_team_goals,0)<0 OR coalesce(p_opponent_goals,0)<0 OR coalesce(p_team_goals,0)>20 OR coalesce(p_opponent_goals,0)>20 THEN RAISE EXCEPTION ''Placar inválido.'';END IF; IF coalesce(p_goals,0)>coalesce(p_team_goals,0) OR coalesce(p_assists,0)>coalesce(p_team_goals,0) THEN RAISE EXCEPTION ''Estatísticas individuais incompatíveis com o placar.'';END IF; IF coalesce(p_played,false) AND coalesce(p_minutes,0)<=0 THEN RAISE EXCEPTION ''Participação em campo exige minutos jogados.'';END IF; IF coalesce(p_started,false) AND NOT coalesce(p_played,false) THEN RAISE EXCEPTION ''Titular precisa registrar participação em campo.'';END IF; IF NOT coalesce(p_played,false) AND p_rating IS NOT NULL THEN RAISE EXCEPTION ''Jogador sem participação não pode receber nota.'';END IF;';
BEGIN
