-- Keep the in-person arrival interaction explicitly identified as the captain.
...
-- Attribute growth from career training must also refresh the player's OVR.
...
BEGIN
  ...
  FROM public.jogadores WHERE id=p_player_id;
  ...
  IF v_gain>0 THEN
    ...
    UPDATE public.jogadores
    SET ovr=public.calculate_player_ovr(atributos),
        updated_at=now()
    WHERE id=p_player_id;
